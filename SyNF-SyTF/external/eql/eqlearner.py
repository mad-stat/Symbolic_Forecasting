...
f_dict_sy = {
    ...
    "diff": lambda x: sy.Function("diff")(x),
...
f_dict_torch = {
    ...
    "diff": (lambda x: x[1:] - x[:-1] if len(x.shape) == 1 else x[:, 1:] - x[:, :-1], 1),
...
class EQL(torch.nn.Module):
    ...
    def update_masks(self, threshold):
        #Updates masks, i.e. masked unit will remain masked and cannot become unmasked as in the `set_masks`method.
